    ...
    >
      <span
        className={`absolute top-1 left-0 w-5 h-5 rounded-full bg-white shadow transition-transform duration-200 ${
          value ? 'translate-x-[2.125rem]' : 'translate-x-1'
        }`}
      />
    </button>
  );
...
export default function SettingsPanel({ sendMessage, isLoaded }) {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[240] animate-overlay-in pointer-events-none">
          <div className="absolute inset-0 bg-[rgba(2,11,26,0.55)]" />
          <div className="absolute inset-0 bg-grid-neon animate-grid-drift opacity-40" />
        </div>
      )}
          ...
          <div className={`relative flex flex-col flex-1 overflow-y-auto px-9 py-8 touch:px-5 touch:py-5 ${!isLoaded ? 'pointer-events-none' : ''}`}>
            ...
            <div className="flex items-center justify-between mb-7 touch:mb-4">
              ...
              <button
                ...
                className="pointer-events-auto bg-transparent border-none text-[rgba(0,180,255,0.45)] text-xl cursor-pointer transition-colors hover:text-white font-mono"
              >
                ✕
              </button>
            </div>
            ...
            {/* Not loaded warning */}
            {!isLoaded && (
              <p className="font-mono text-sm tracking-wide text-[rgba(243,156,18,0.8)] mb-5 text-center animate-blink pointer-events-none">
            ...
            <div className={!isLoaded ? 'opacity-40' : ''}>
              ...
              <div className="flex items-center justify-between">
                <span className="font-mono text-base text-[rgba(180,220,255,0.85)] tracking-wide">
                  Couleurs de statut
                </span>
                <Toggle

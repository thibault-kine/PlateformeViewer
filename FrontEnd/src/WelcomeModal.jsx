...
function Divider() {
  return (
    <div className="h-px bg-gradient-to-r from-transparent via-[rgba(0,160,255,0.4)] to-transparent my-7 touch:my-4" />
  );
}
    ...
    >
      ...
      <div className="absolute inset-0 overflow-y-auto">
        <div className="relative flex items-center justify-center w-full min-h-full py-4">
          <div
            className="relative w-full max-w-[900px] mx-4 my-auto bg-[rgba(3,14,36,0.96)] border border-[rgba(0,160,255,0.3)] rounded-xl px-14 py-12 touch:px-5 touch:py-8 overflow-hidden shadow-neon-modal animate-modal-in"
          ...
          >
            ...
            <div className="relative flex flex-col items-center gap-3 mb-1">
              <p className="m-0 font-mono text-base touch:text-xs tracking-[0.45em] text-[rgba(0,180,255,0.6)] uppercase">
                Bienvenue dans
              </p>
              ...
              <p className="m-0 font-mono text-base touch:text-xs tracking-[0.35em] text-[rgba(0,180,255,0.55)] uppercase">
            ...
            {/* Description */}
            <p className="relative m-0 font-mono text-lg touch:text-sm leading-relaxed tracking-wide text-[rgba(200,230,255,0.9)] text-center">
              Ce projet est un{' '}
            ...
            <div className="relative flex flex-col gap-4">
              {FEATURES.map(({ icon, title, desc }) => (
                <div
                  key={title}
                  className="flex items-start gap-5 touch:gap-3 bg-[rgba(0,30,70,0.45)] border border-[rgba(0,140,255,0.18)] rounded-lg px-6 py-4 touch:px-4 touch:py-3"
                >
                  <span className="text-neon-blue text-3xl touch:text-xl mt-1 shrink-0">{icon}</span>
                  <div>
                    <p className="m-0 font-mono text-xl touch:text-base font-bold tracking-wide text-white mb-1">
                      {title}
                    </p>
                    <p className="m-0 font-mono text-base touch:text-sm leading-relaxed text-[rgba(180,220,255,0.75)]">
                      {desc}
                    </p>
                  </div>
                </div>
            ...
            <div className="relative flex flex-col items-center gap-4">
              <button
                onClick={() => setOpen(false)}
                className="px-16 py-4 touch:px-8 touch:py-3 bg-transparent border border-[rgba(0,180,255,0.6)] rounded-sm font-mono text-xl touch:text-base tracking-[0.35em] text-neon-blue cursor-pointer shadow-neon-btn hover:bg-[rgba(0,120,255,0.12)] hover:border-[rgba(0,220,255,0.9)] hover:text-white hover:shadow-neon-btn-hover active:scale-95 transition-all duration-200"
              >
                EXPLORER
              </button>
              <p className="m-0 font-mono text-sm touch:text-xs tracking-[0.2em] text-white/40">
